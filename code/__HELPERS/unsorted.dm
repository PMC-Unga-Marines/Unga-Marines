...
/proc/Get_Angle(atom/start, atom/end)//For beams.
	...
	if(!start.z)
		...
		if(!start)
			CRASH("Get_Angle called for inexisting atoms (start): [isnull(start.loc) ? start.loc : "null loc"] [start] to [isnull(end.loc) ? end.loc : "null loc"] [end].") //Atoms are not on turfs.
	if(!end.z)
		end = get_turf(end)
		if(!end)
			CRASH("Get_Angle called for inexisting atoms (end): [isnull(start.loc) ? start.loc : "null loc"] [start] to [isnull(end.loc) ? end.loc : "null loc"] [end].") //Atoms are not on turfs.
