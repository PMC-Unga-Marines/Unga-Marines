...
/datum/effects/proc/process_mob()
	...
	if((flags & DEL_ON_UNDEFIBBABLE) && ishuman(affected_atom))
		...
		if(HAS_TRAIT(H, TRAIT_UNDEFIBBABLE)&& H.stat == DEAD)
			qdel(src)
			return FALSE
...
/datum/effects/proc/process_obj()
	...
	if((flags & DEL_ON_DEATH) && affected_obj.obj_integrity <= 0)
		qdel(src)
		return FALSE

	if((flags & DEL_ON_LIVING) && affected_obj.obj_integrity > 0)
		qdel(src)
		return FALSE

	if((flags & NO_PROCESS_ON_DEATH) && affected_obj.obj_integrity <= 0)
		return FALSE
	return TRUE
