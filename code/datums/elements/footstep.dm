...
/datum/element/footstep/proc/prepare_step(mob/living/source)
	...
	if(source.buckled || source.throwing || HAS_TRAIT(source, TRAIT_MOVE_VENTCRAWLING) || source.lying_angle || CHECK_MULTIPLE_BITFIELDS(source.pass_flags, HOVERING) || HAS_TRAIT(source, TRAIT_IMMOBILE))
		return
