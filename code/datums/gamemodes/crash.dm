...
/datum/game_mode/infestation/crash/proc/balance_scales()
	...
	if(xenomorphs_below_ratio >= 1)
		...
		return
	if(xeno_hive.get_total_xeno_number() >= 2 && xenomorphs_below_ratio < 0) // delete the exceeding slots
		xeno_job.remove_job_positions(-xenomorphs_below_ratio - xeno_hive.get_total_xeno_number())
		xeno_hive.update_tier_limits()
		return
