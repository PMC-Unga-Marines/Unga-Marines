...
/datum/component/jump/proc/do_jump(mob/living/jumper)
	...
	if(stamina_cost && (jumper.get_stamina_loss() > -stamina_cost))
	...
	jumper.adjust_stamina_loss(stamina_cost)
	...
	animate(jumper, pixel_y = jumper.pixel_y + effective_jump_height, layer = MOB_JUMP_LAYER, time = effective_jump_duration * 0.5, easing = CIRCULAR_EASING|EASE_OUT, flags = ANIMATION_PARALLEL)
	animate(pixel_y = jumper.pixel_y - effective_jump_height, layer = original_layer, time = effective_jump_duration * 0.5, easing = CIRCULAR_EASING|EASE_IN)

	if(jump_flags & JUMP_SHADOW)
		animate(shadow_filter, y = -effective_jump_height, size = 4, time = effective_jump_duration * 0.5, easing = CIRCULAR_EASING|EASE_OUT, flags = ANIMATION_PARALLEL)
		animate(y = 0, size = 0.9, time = effective_jump_duration * 0.5, easing = CIRCULAR_EASING|EASE_IN)
