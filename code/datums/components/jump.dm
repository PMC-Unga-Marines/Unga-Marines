...
/datum/component/jump/proc/do_jump(mob/living/jumper)
	...
	animate(jumper, pixel_z = jumper.pixel_z + effective_jump_height, layer = max(MOB_JUMP_LAYER, original_layer), time = effective_jump_duration * 0.5, easing = CIRCULAR_EASING|EASE_OUT)
	animate(pixel_z = jumper.pixel_z - effective_jump_height, layer = original_layer, time = effective_jump_duration * 0.5, easing = CIRCULAR_EASING|EASE_IN)
