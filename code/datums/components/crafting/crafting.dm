...
/datum/component/personal_crafting/proc/get_environment(atom/a, list/blacklist = null, radius_range = 1)
	...
	for(var/atom/movable/AM in range(radius_range, a))
		...
		if(isitem(AM))
			...
			if(item.item_flags & ITEM_ABSTRACT) //let's not tempt fate, shall we?
				continue
		. += AM
...
/datum/component/personal_crafting/proc/check_tools(atom/source, datum/crafting_recipe/recipe, list/surroundings)
	...
	for(var/obj/item/contained_item in source.contents)
		if(contained_item.storage_datum)
...
/datum/component/personal_crafting/proc/construct_item(atom/crafter, datum/crafting_recipe/recipe)
	...
	if(recipe.crafting_flags & CRAFT_ONE_PER_TURF)
...
/datum/component/personal_crafting/proc/del_reqs(datum/crafting_recipe/R, atom/a)
				...
				while(amt > 0)
					...
					if(reagent_volume)
						...
						if(reagent_volume >= amt)
							RC.reagents.trans_to(holder, amt, no_react = TRUE)
							continue main_loop
						else
							RC.reagents.trans_to(holder, reagent_volume, no_react = TRUE)
							surroundings -= RC
							amt -= reagent_volume
	...
	while(Deletion.len)
		...
		// Snowflake handling of storage objects.
		...
		if(isstorageobj(DL))
			...
			container.storage_datum.quick_empty(DL, a) //TODO: port remove_all() from TG that allows us to do this without a user
		qdel(DL)
