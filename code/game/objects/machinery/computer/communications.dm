	...
	switch(state)
		if(STATE_DEFAULT)
			if(authenticated)
				...
				if(authenticated == 2)
					...
					dat += length(GLOB.admins) > 0 ? "<BR>\[ <A href='byond://?src=[text_ref(src)];operation=messageTGMC'>Send a message to TGMC</A> \]" : "<BR>\[ TGMC communication offline \]"
		...
		if(STATE_ALERT_LEVEL)
			...
			if(GLOB.marine_main_ship.security_level == SEC_LEVEL_DELTA)
				...
				dat += "<A href='byond://?src=[text_ref(src)];operation=securitylevel;newalertlevel=[SEC_LEVEL_BLUE]'>Blue</A><BR>"
				dat += "<A href='byond://?src=[text_ref(src)];operation=securitylevel;newalertlevel=[SEC_LEVEL_GREEN]'>Green</A>"
