...
/obj/machinery/computer/skills/interact(mob/user)
	...
	var/dat
	if(temp)
		dat = "[temp]<BR><BR><A href='byond://?src=[text_ref(src)];choice=Clear Screen'>Clear Screen</A>"
	else
		...
		if (authenticated)
			switch(screen) //TODO: this is awful.
				...
				if(2)
					...
					dat += "<BR><BR>\n<A href='byond://?src=[text_ref(src)];choice=Print Record'>Print Record</A><BR>\n<A href='byond://?src=[text_ref(src)];choice=Return'>Back</A><BR>"
				if(3)
					if(!length(Perp))
...
/obj/machinery/computer/skills/Topic(href, href_list)
	...
	switch(href_list["choice"])
		...
		if("Search Records")
			...
			if(topic_field_checks(t1, usr, null, null))
				return
			Perp = list()
			t1 = lowertext(t1)
...
/obj/machinery/computer/skills/proc/topic_field_checks(input, mob/user, record, old_record)
	if(!input)
		return FALSE
	if(!authenticated)
		return FALSE
