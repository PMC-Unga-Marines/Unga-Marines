...
/obj/effect/rappel_rope/tadpole/attack_hand(mob/living/user)
	...
	if(!step(user, get_dir(user, src)))
		user.balloon_alert(user, "Что-то мешает вам использовать трос!")
		return
	user.balloon_alert_to_viewers("[user] начинает взбираться по тросу...", "Вы начинаете взбираться по тросу...")
...
/obj/item/spare_cord
	name = "Коробка сменного десантного троса"
	desc = "Коробка с дорогим запасным десантным шнуром, пропитанным пласталью, для десантно-канатной системы. Нажмите на десантно-канатную-систему, чтобы заменить поврежденный шнур и вернуть системе работоспособность."
	icon = 'icons/obj/structures/prop/mainship.dmi'
	icon_state = "cordbox"
