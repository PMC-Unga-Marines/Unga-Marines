/client/proc/cmd_mass_modify_object_variables(atom/A, var_name)
	...
	if(A?.type)
		strict_type  = vv_subtype_prompt(A.type)

	src.massmodify_variables(A, var_name, strict_type)
...
/client/proc/massmodify_variables(datum/O, var_name = "", strict_type = FALSE)
	if(!check_rights(R_VAREDIT))
		return
	if(!istype(O))
		return
	...
	switch(class)
		if(VV_RESTORE_DEFAULT)
			...
			var/list/items = get_all_of_type(O.type, strict_type)
