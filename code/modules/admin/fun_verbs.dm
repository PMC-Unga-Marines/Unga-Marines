...
/datum/admins/proc/command_report()
	...
	switch(tgui_alert(usr, "Should this be announced to the general population?", "Announce", list("Yes", "No", "Cancel")))
		...
		if("No")
			priority_announce("Новое объявление доступно на всех консолях связи.", "Оповещение Всему Экипажу", type = ANNOUNCEMENT_PRIORITY, sound = 'sound/AI/commandreport.ogg')
		else
			return
