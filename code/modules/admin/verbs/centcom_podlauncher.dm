...
/datum/centcom_podlauncher/ui_act(action, list/params)
	...
	switch(action)
		////////////////////////////UTILITIES//////////////////
		if("bay1")
			bay = locate(/area/centcom/supplypod/loading/one) in get_sorted_areas() //set the "bay" variable to the corresponding room in centcom
			refreshBay() //calls refreshBay() which "recounts" the bay to see what items we can launch (among other things).
			. = TRUE
		if("bay2")
			bay = locate(/area/centcom/supplypod/loading/two) in get_sorted_areas()
			refreshBay()
			. = TRUE
		if("bay3")
			bay = locate(/area/centcom/supplypod/loading/three) in get_sorted_areas()
			refreshBay()
			. = TRUE
		if("bay4")
			bay = locate(/area/centcom/supplypod/loading/four) in get_sorted_areas()
			refreshBay()
			. = TRUE
		if("bay5")
			bay = locate(/area/centcom/supplypod/loading/ert) in get_sorted_areas()
			refreshBay()
			. = TRUE
		if("teleportCentcom") //Teleports the user to the centcom supply loading facility.
			...
			var/area/A = locate(bay) in get_sorted_areas()
			var/list/turfs = list()
			for(var/turf/T in A)
		...
		if("setstyle")
			var/found = FALSE
			for(var/list/style in GLOB.pod_styles)
				if(params["style"] == style[1])
					temp_pod.setStyle(style[4])
					found = TRUE
					break
			if(!found)
				CRASH("tried to set an invalid pod style")
			. = TRUE
