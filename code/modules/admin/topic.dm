	...
	if(href_list["ahelp"])
		...
		if(isliving(M))
			...
			switch(L.stat)
				...
				if(DEAD)
					status = "Dead"
			health = "Oxy: [L.getOxyLoss()]  Tox: [L.getToxLoss()]  Fire: [L.get_fire_loss()]  Brute: [L.get_brute_loss()]  Clone: [L.getCloneLoss()]  Brain: [L.getBrainLoss()]  Stamina: [L.getStaminaLoss()]"
