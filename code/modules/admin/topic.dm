	...
	if(href_list["ahelp"])
		...
		if(isliving(M))
			...
			switch(L.stat)
				...
				if(DEAD)
					status = "Dead"
			health = "Oxy: [L.get_oxy_loss()]  Tox: [L.get_tox_loss()]  Fire: [L.get_fire_loss()]  Brute: [L.get_brute_loss()]  Clone: [L.getCloneLoss()]  Brain: [L.getBrainLoss()]  Stamina: [L.getStaminaLoss()]"
...
[span_admin("<span class='message'>[ADMIN_FULLMONTY(M)]")]</span><hr></span>"})
	...
	if(href_list["evac_authority"])
		...
		switch(href_list["evac_authority"])
			...
			if("toggle_evac")
				SSevacuation.scuttle_flags ^= EVACUATION_DENY_flags
				log_admin("[key_name(src)] has [SSevacuation.scuttle_flags & EVACUATION_DENY_flags ? "forbidden" : "allowed"] ship-wide evacuation.")
				message_admins("[ADMIN_TPMONTY(usr)] has [SSevacuation.scuttle_flags & EVACUATION_DENY_flags ? "forbidden" : "allowed"] ship-wide evacuation.")
			...
			if("toggle_dest")
				SSevacuation.scuttle_flags ^= SELF_DESTRUCT_DENY_flags
