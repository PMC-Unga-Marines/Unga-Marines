...
/mob/living/proc/ff_check(total_damage, mob/living/victim)
	...
	if(friendly_fire[FF_DAMAGE_OUTGOING] < ff_limit)
		return
	send2adminchat("ROUND:[GLOB.round_id] | FF ALERT", "[key_name(src)] was kicked for excessive friendly fire. [friendly_fire[FF_DAMAGE_OUTGOING]] damage witin [ff_cooldown * 0.1] seconds")
	create_message("note", ckey(client.key), "SYSTEM", "Autokicked due to excessive friendly fire. [friendly_fire[FF_DAMAGE_OUTGOING]] damage within [ff_cooldown * 0.1] seconds.", null, null, FALSE, FALSE, null, FALSE, "Minor")
