/datum/supply_packs/personal/leader
	job_type = /datum/job/terragov/squad/leader
	containertype = /obj/structure/closet/crate/weapon

/datum/supply_packs/personal/leader/masks
	name = "SWAT protective mask"
	contains = list(/obj/item/clothing/mask/gas/swat)
	cost = 5

/datum/supply_packs/personal/leader/marine_shield
	name = "TL-172 defensive shield"
	contains = list(/obj/item/weapon/shield/riot/marine)
	cost = 3

/datum/supply_packs/personal/leader/marine_shield/deployable
	name = "TL-182 deployable shield"
	contains = list(/obj/item/weapon/shield/riot/marine/deployable)
	cost = 1

/datum/supply_packs/personal/leader/grenade_belt
	name = "High capacity grenade belt"
	contains = list(/obj/item/storage/belt/grenade/b17)
	cost = 5

/datum/supply_packs/personal/leader/imager_goggle
	name = "Optical imager goggles"
	contains = list(/obj/item/clothing/glasses/night/imager_goggles)
	cost = 4

/datum/supply_packs/personal/leader/night_vision_batteries
	name = "Double pack of night vision batteries"
	contains = list(/obj/item/cell/night_vision_battery, /obj/item/cell/night_vision_battery)
	cost = 3

/datum/supply_packs/personal/leader/stickytanglenades
	name = "M45-T adhesive tanglefoot grenade"
	notes = "Contains 25 M45-T sticky grenade"
	contains = list(/obj/item/storage/box/visual/grenade/drain/sticky)
	cost = 8

/datum/supply_packs/personal/leader/explosives_sticky
	name = "M40 adhesive charge grenade box"
	notes = "Contains 25 grenades"
	contains = list(/obj/item/storage/box/visual/grenade/sticky)
	cost = 8

/datum/supply_packs/personal/leader/explosives_hedp
	name = "M40 HEDP high explosive grenade box"
	notes = "Contains 25 grenades"
	contains = list(/obj/item/storage/box/visual/grenade/frag)
	cost = 8

/datum/supply_packs/personal/leader/explosives_cloaker
	name = "M45 cloaker grenade box"
	notes = "Contains 25 grenades"
	contains = list(/obj/item/storage/box/visual/grenade/cloaker)
	cost = 8

/datum/supply_packs/personal/leader/explosives_cloak
	name = "M40-2 SCDP grenade box"
	notes = "Contains 25 grenades"
	contains = list(/obj/item/storage/box/visual/grenade/cloak)
	cost = 8

/datum/supply_packs/personal/leader/explosives_lasburster
	name = "M80 lasburster grenade box"
	notes = "Contains 25 grenades"
	contains = list(/obj/item/storage/box/visual/grenade/lasburster)
	cost = 8

/datum/supply_packs/personal/leader/explosives_hidp
	name = "M40 HIDP incendiary explosive grenade box"
	notes = "Contains 25 grenades"
	contains = list(/obj/item/storage/box/visual/grenade/incendiary)
	cost = 9

/datum/supply_packs/personal/leader/explosives_m15
	name = "M15 fragmentation grenade box"
	notes = "Contains 15 grenades"
	contains = list(/obj/item/storage/box/visual/grenade/m15)
	cost = 9

/datum/supply_packs/personal/leader/explosives_trailblazer
	name = "M45 trailblazer grenade box"
	notes = "Contains 25 grenades"
	contains = list(/obj/item/storage/box/visual/grenade/trailblazer)
	cost = 9

/datum/supply_packs/personal/leader/explosives_hefa
	name = "M25 HEFA grenade box"
	notes = "Contains 25 grenades"
	contains = list(/obj/item/storage/box/visual/grenade/hefa)
	cost = 14

/datum/supply_packs/personal/leader/explosives_plasmadrain
	name = "M40-T gas grenade box"
	notes = "Contains 25 grenades"
	contains = list(/obj/item/storage/box/visual/grenade/drain)
	cost = 18

/datum/supply_packs/personal/leader/plastique
	name = "C4 plastic explosive"
	contains = list(/obj/item/explosive/plastique)
	cost = 1

/datum/supply_packs/personal/leader/rpgoneuse
	name = "RL-72 disposable RPG"
	contains = list(/obj/item/weapon/gun/launcher/rocket/oneuse)
	cost = 5

/datum/supply_packs/personal/leader/standard_ammo
	name = "Surplus standard ammo crate"
	notes = "Contains 22 ammo boxes of a wide variety which come prefilled. You lazy bum."
	contains = list(/obj/structure/largecrate/supply/ammo/standard_ammo)
	containertype = null
	cost = 3

/datum/supply_packs/personal/leader/beacons_supply
	name = "Supply beacon"
	contains = list(/obj/item/supply_beacon)
	cost = 3

/datum/supply_packs/personal/leader/fulton_extraction_pack
	name = "Fulton extraction pack"
	contains = list(/obj/item/fulton_extraction_pack)
	cost = 2
