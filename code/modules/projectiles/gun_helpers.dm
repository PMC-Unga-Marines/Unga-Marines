...
/obj/item/weapon/gun/afterattack(atom/target, mob/user, has_proximity, click_parameters)
	...
	if(islascell(target))
		...
		if(CHECK_BITFIELD(get_magazine_features_flags(cell_to_reload), MAGAZINE_WORN))
			return
		tactical_reload(target, user)
...
/obj/item/weapon/gun/proc/tactical_reload(obj/item/new_magazine, mob/living/carbon/human/user)
	...
	var/tac_reload_time = max(0.25 SECONDS, 0.75 SECONDS - (user.skills.getRating(SKILL_FIREARMS) + 1) * 5)
...
/obj/item/weapon/gun/proc/add_firemode(added_firemode, mob/user)
	...
	switch(length(gun_firemode_list))
		...
		if(2)
			...
			if(user)
				var/mob/living/living_user = user
				if((src == living_user.l_hand || src == living_user.r_hand) && !CHECK_BITFIELD(deploy_flags, IS_DEPLOYED))
...
/obj/item/weapon/gun/proc/toggle_auto_aim_mode(mob/living/carbon/human/user) //determines whether toggle_aim_mode activates at the end of gun/wield proc
	if((item_flags & FULLY_WIELDED) || (deploy_flags & IS_DEPLOYED)) //if gun is wielded it toggles aim mode directly instead
...
/obj/item/weapon/gun/proc/toggle_aim_mode(mob/living/carbon/human/user)
	var/static/image/aim_mode_visual = image('icons/mob/hud/human_misc.dmi', null, "aim_mode")
...
/obj/item/weapon/gun/proc/activate_rail_attachment()
	SIGNAL_HANDLER
	if(gun_user?.get_active_held_item() != src && !(deploy_flags & IS_DEPLOYED))
		return
