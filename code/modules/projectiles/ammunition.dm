...
/obj/item/ammo_magazine/flamer_fire_act(burnlevel, flame_color)
	if(!current_rounds)
		return
	cell_explosion(loc, 5, 5)
	qdel(src)
...
/obj/item/big_ammo_box/flamer_fire_act(burnlevel, flame_color)
	if(!bullet_amount)
		return
	cell_explosion(loc, 15, 15)
	qdel(src)
