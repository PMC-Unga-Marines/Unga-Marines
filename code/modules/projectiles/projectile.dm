	...
	for(var/i in 1 to required_moves)
		...
		if(ISDIAGONALDIR(movement_dir)) //Diagonal case. We need to check the turf to cross to get there.
			...
			for(var/j in uncross_scheduled)
				...
				if(ammo.ammo_behavior_flags & AMMO_PASS_THROUGH_MOVABLE && !CHECK_BITFIELD(thing_to_uncross.resistance_flags, BLOCK_PASSTHROUGH_PROJECTILES) || ismob(thing_to_uncross) && CHECK_BITFIELD(ammo.ammo_behavior_flags, AMMO_PASS_THROUGH_MOB))
...
/mob/living/bullet_act(obj/projectile/proj)
	...
	if(damage > 0)
		...
		apply_damage(damage, proj.ammo.damage_type, proj.def_zone, blocked = proj.armor_type, updating_health = TRUE) //This could potentially delete the source.
	else
