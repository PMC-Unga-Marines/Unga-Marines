...
/obj/machinery/atm/attack_hand(mob/living/user)
	...
	if(ticks_left_locked_down > 0)
		...
		if(authenticated_account.suspended)
			...
			switch(view_screen)
				...
				if(TRANSFER_FUNDS)
					...
					dat += "<form name='transfer' action='byond://?src=[text_ref(src)]' method='get'>"
					...
					dat += "<form name='withdrawal' action='byond://?src=[text_ref(src)]' method='get'>"
					dat += "<input type='hidden' name='src' value='\ref[src]'>"
		...
		dat += "<form name='atm_auth' action='byond://?src=[text_ref(src)]' method='get'>"
		dat += "<input type='hidden' name='src' value='\ref[src]'>"
