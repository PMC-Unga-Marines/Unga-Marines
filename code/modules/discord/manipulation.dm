...
/datum/admins/proc/discord_manipulation()
	...
	switch(lookup_choice)
		if("ID")
			var/lookup_id = tgui_input_text(usr,"Enter Discord ID to lookup ckey") as text|null
		...
		if("Ckey")
			var/lookup_ckey = tgui_input_text(usr,"Enter Ckey to lookup ID") as text|null
