...
/datum/reagent/medicine/oxycodone/on_mob_life(mob/living/L, metabolism)
	...
	if(iscarbon(L))
		var/mob/living/carbon/C = L
		C.set_shock_stage(min(C.shock_stage - 1*effect_str, 150)) //you can still paincrit if under ludicrous situations, but you can't go into deep shock
	return ..()
...
/datum/reagent/medicine/neuraline/on_mob_life(mob/living/L)
	...
	if(iscarbon(L))
		var/mob/living/carbon/C = L
		C.set_shock_stage(min(C.shock_stage - volume*effect_str, 150)) //will pull a target out of deep paincrit instantly, if he's in it
	return ..()
...
/datum/reagent/medicine/russian_red/on_mob_life(mob/living/L, metabolism)
	...
	if(iscarbon(L))
		var/mob/living/carbon/C = L
		C.set_shock_stage(min(C.shock_stage - 5*effect_str, 150)) //removes a target from deep paincrit instantly
	return ..()
