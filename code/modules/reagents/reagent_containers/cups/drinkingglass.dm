...
/obj/item/reagent_containers/cup/glass/drinking_glass/Initialize(mapload, vol)
	. = ..()
	AddComponent( \
...
/obj/item/reagent_containers/cup/glass/drinking_glass/on_cup_change(datum/glass_style/style)
	. = ..()
	fill_icon_thresholds = null

// And having our icon reset restores our fill thresholds
/obj/item/reagent_containers/cup/glass/drinking_glass/on_cup_reset()
	. = ..()
	fill_icon_thresholds ||= list(0)
...
/obj/item/reagent_containers/cup/glass/drinking_glass/proc/on_cleaned(obj/source_component, obj/source)
	SIGNAL_HANDLER
	if(!HAS_TRAIT(src, TRAIT_WAS_RENAMED))
		return
...
/obj/item/reagent_containers/cup/glass/drinking_glass/shotglass/update_name(updates)
	if(HAS_TRAIT(src, TRAIT_WAS_RENAMED))
		return
	. = ..()
	name = "[length(reagents.reagent_list) ? "filled " : ""]shot glass"

/obj/item/reagent_containers/cup/glass/drinking_glass/shotglass/update_desc(updates)
	if(HAS_TRAIT(src, TRAIT_WAS_RENAMED))
		return
	. = ..()
...
/obj/item/reagent_containers/cup/glass/drinking_glass/filled
	base_container_type = /obj/item/reagent_containers/cup/glass/drinking_glass

/obj/item/reagent_containers/cup/glass/drinking_glass/filled/Initialize(mapload, vol)
	. = ..()
	update_appearance()
