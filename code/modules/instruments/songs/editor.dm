...
/datum/song/proc/instrument_status_ui()
	...
	switch(sustain_mode)
		if(SUSTAIN_LINEAR)
			smt = "Linear"
			modetext = "<a href='byond://?src=[REF(src)];setlinearfalloff=1'>Linear Sustain Duration</a>: [sustain_linear_duration * 0.1] seconds<br>"
...
/datum/song/Topic(href, href_list)
	...
	if(!.)
		return/*
	if(!usr.can_interact(parent, TRUE, FALSE, FALSE, FALSE))
		usr << browse(null, "window=instrument")
		usr.unset_machine()
		return*/

	parent.add_fingerprint(usr, "interacted with")
