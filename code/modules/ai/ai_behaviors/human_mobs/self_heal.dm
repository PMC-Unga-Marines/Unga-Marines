...
/datum/ai_behavior/human/proc/try_heal()
	...
	var/list/dam_list = list(
		BRUTE = living_parent.get_brute_loss(),
		BURN = living_parent.get_fire_loss(),
		TOX = living_parent.get_tox_loss(),
		OXY = living_parent.get_oxy_loss(),
		CLONE = living_parent.get_clone_loss(),
		PAIN = 0,
	)
